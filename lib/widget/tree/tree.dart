...
class _FlutterTreeProState extends State<FlutterTreePro> {
    ...
    if (widget.config.dataType == DataType.DataList) {
      ...
      for (var item in widget.initialListData) {
        for (var element in treeMap.values.toList()) {
          if (item['waretypeId'] == element['waretypeId']) {
  ...
  Widget buildCheckBoxIcon(Map<String, dynamic> e) {
    if(widget.single && e['children'] != null){
  ...
  selectCheckedBox(Map<String, dynamic> dataModel) {
    ...
    if(widget.single){
      if (checked == 2) {
        ...
        List? list = treeMap.values.toList();
        list.forEach((value) {
          value['checked'] = 0;
        });
        dataModel['checked'] = 2;
